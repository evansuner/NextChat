...
1. 安装 Node.js 18 和 Bun，具体细节请询问 ChatGPT；
2. 执行 `bun install && bun run dev` 即可。⚠️ 注意：此命令仅用于本地开发，不要用于部署！
3. 如果你想本地部署，请使用 `bun install && bun run build && bun run start` 命令，你可以配合 pm2 来守护进程，防止被杀死，详情询问 ChatGPT。
